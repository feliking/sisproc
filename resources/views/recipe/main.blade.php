        ...
        <div class="col-md-12">
            <div class="card">
                <div class="card-header">
                    <strong class="card-title">Lista de Recetas</strong>
                    <button type="button" class="btn btn-success" data-toggle="modal" data-target="#add_receta" title="Añadir Receta"><i class="fa fa-plus"></i>Añadir Receta</button>
                    ...
                    <table id="bootstrap-data-table-export" class="table table-striped table-bordered dt-responsive nowrap">
                        <thead>
                            <tr>
                                ...
                                <th v-for="(colorant, index) in colorants" class="bg-danger text-light">@{{ colorant.name }}</th>
                                <th v-for="(assistant, index) in assistants" class="bg-warning">@{{ assistant.name }}</th>
                            </tr>
                        </thead>
                        <tbody>
                            <tr v-for="(recipe, index) in recipes">
                                ...
                                <td>@{{ recipe.codigo }}</td>
                                <td v-for="colorant in recipe.colorants">@{{ colorant.valor }}</td>
                                <td v-for="assistant in recipe.assistants">@{{ assistant.valor }}</td>
                            </tr>
                        </tbody>
                    </table>
                </div>
    ...
    const app = new Vue({
        ...
        mounted() {
            axios.get('/recipe').then(response => {
                ...
                this.colorants = response.data[1];
                this.assistants = response.data[2]
